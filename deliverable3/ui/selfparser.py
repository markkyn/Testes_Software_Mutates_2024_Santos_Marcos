...
# Special functions
from functions.power_function import PowerFunction
from functions.log import LogFunction
from functions.exponential_function import ExponentialFunction
#from functions.standard_deviation import StandardDeviation
from functions.mean_absolute_deviation import MeanAbsoluteDeviation
from functions.hyperbolic_sine import HyperbolicSine
...
acos_regex = re.compile(r'(?:aCos\(\-*\d*\.*\d*\))')
...
def parseEqu(s):
    ...
    while True:
        ...
        elif acos_regex.findall(_local_string):
            matc = acos_regex.findall(_local_string)
            for matches in matc:
                values = param_regex.findall(matches)  # expecting 1 value
                __acos.set_values(values[0])
                ev = str(round(__acos.calculate_acos(), 2))
                _local_string = _local_string.replace(matches, ev)
            print("acos changed string to -- " + _local_string)
